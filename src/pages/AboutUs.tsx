...
const leaders = [
  {
    ...
    bioEn: [
      ...
      "Dr. Al Mazeedi is committed to transforming the healthcare landscape in Kuwait. He has spearheaded numerous initiatives aimed at integrating cutting-edge technology into healthcare delivery systems, improving patient outcomes, and enhancing overall efficiency. His visionary leadership and unwavering commitment to innovation have led to the establishment of several groundbreaking projects, earning him accolades and recognition from his peers and the wider medical community.",
    ],
    bioAr: [
  ...
  },
  {
    ...
    nameEn: "Shibu Mathew",
    nameAr: "شيبو ماثيو",
    roleEn: "Chief Financial Officer and Director of HR",
    roleAr: "المدير المالي الرئيسي ومدير الموارد البشرية",
    credentialsEn: "",
    credentialsAr: "",
    bioEn: [
      "Shibu Mathew first joined Royale Hayat Hospital in 2006 as a member of the pre-opening team. His dedication and expertise quickly led to his appointment as Financial Controller in 2007, followed by his promotion to Chief Financial Officer in 2010. In his role, Mr. Shibu is responsible for long-range investment planning, performance analysis, and budget evaluation for all Group companies. Additionally, he serves as a Board Member for many of these subsidiary companies, contributing to their strategic direction and financial health.",
      "Before his tenure at Royale Hayat Hospital, Mr. Shibu gained valuable experience as the Accounting, Finance, and Treasury Manager for various multinational companies. His extensive expertise encompasses the design of accounting systems and policies, the mapping and roll-out of ERP packages, feasibility assessments, financial and treasury management, as well as the development of timely statutory, management, and shareholder reports.",
      "Mr. Shibu's qualifications include being a Member of the Certified Management Accountant (CMA, USA) and an Associate Member of the Institute of Cost Accountants of India (ICMA, India). He holds a Diploma in International Financial Reporting Standards and has completed Continuing Professional Education from the Chartered Institute of Personnel and Development, U.K. Furthermore, he has pursued additional training in Health Care Executives and Strategy from Harvard T.H. Chan School of Public Health.",
    ],
    bioAr: [
      "انضم شيبو ماثيو لأول مرة إلى مستشفى رويال حياة في عام 2006 كعضو في فريق ما قبل الافتتاح. أدى تفانيه وخبرته سريعاً إلى تعيينه كمراقب مالي في 2007، تلاه ترقيته إلى المدير المالي الرئيسي في 2010. يتولى مسؤولية التخطيط الاستثماري طويل الأجل وتحليل الأداء وتقييم الميزانية لجميع شركات المجموعة.",
      "قبل عمله في مستشفى رويال حياة، اكتسب خبرة قيمة كمدير للمحاسبة والمالية والخزينة في شركات متعددة الجنسيات. تشمل خبرته الواسعة تصميم أنظمة وسياسات المحاسبة وتقييم الجدوى والإدارة المالية وإعداد التقارير.",
      "تشمل مؤهلاته عضوية محاسب الإدارة المعتمد (CMA، الولايات المتحدة) وعضوية معهد محاسبي التكاليف في الهند. حاصل على دبلوم في معايير التقارير المالية الدولية وأكمل تعليماً مهنياً مستمراً من معهد تشارترد للأفراد والتنمية، المملكة المتحدة، وتدريب في إدارة الرعاية الصحية والاستراتيجية من كلية هارفارد للصحة العامة.",
    ],
  },
  {
    initials: "HG",
    ...
    bioAr: [
      "تخرج الدكتور حامد من كلية الطب المرموقة في جامعة هايدلبرغ في ألمانيا، وبنى مسيرة مهنية مميزة في مجال التخدير والعناية المركزة وإدارة الألم. عمل كاستشاري ومحاضر في جامعة هايدلبرغ. حاصل على شهادة البورد الألماني في التخدير والعناية المركزة الجراحية وإدارة الألم السريري.",
...
const LeaderCard = ({ leader, lang }: { leader: typeof leaders[0]; lang: string }) => {
    ...
    >
      <div className="flex flex-col md:flex-row">
        ...
        <div className="md:w-64 flex-shrink-0 bg-primary/5 flex items-center justify-center p-8 md:p-10">
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-primary/10 flex items-center justify-center border-4 border-primary/20">
        ...
        <div className="flex-1 p-6 md:p-8">
          ...
          <div className={`space-y-3 overflow-hidden transition-all duration-500 ${expanded ? "max-h-[2000px]" : "max-h-[100px]"}`}>
            {bio.map((p, i) => (
              <p key={i} className="font-body text-sm text-muted-foreground leading-relaxed">{p}</p>
            ))}
          </div>
          {bio.length > 1 && (
            <button
...
const AboutUs = () => {
  ...
  return (
    <div className="min-h-screen bg-background pt-[var(--header-height,56px)]">
      ...
      {show("history") && <section className="py-16 bg-background" id="history">
          ...
          <div className="max-w-4xl mx-auto space-y-5">
            {[
              ...
              <motion.p key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                className="font-body text-sm text-muted-foreground leading-relaxed">{p}</motion.p>
            ))}
          </div>
        </div>
      </section>}
      ...
      {/* Leadership Team */}
      {show("leadership") && <section className="py-16 bg-background" id="leadership">
        <div className="container mx-auto px-6">
          <ScrollAnimationWrapper>
            <div className="text-center mb-10">
              <p className="text-accent text-xs tracking-[0.3em] uppercase font-body mb-3">
      ...
      {/* Mission & Values */}
      {show("mission") && <section className="py-16 bg-secondary/10" id="mission">
        <div className="container mx-auto px-6">
          <ScrollAnimationWrapper>
            <div className="text-center mb-10">
              <p className="text-accent text-xs tracking-[0.3em] uppercase font-body mb-3">

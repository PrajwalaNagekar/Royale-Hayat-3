...
const DepartmentDoctors = ({ doctors, lang }: { doctors: typeof allDoctors; lang: string }) => {
  ...
  const showArrows = doctors.length > 4;
      ...
      <div className="container mx-auto px-6">
        <ScrollAnimationWrapper>
          <div className="text-center mb-8">
            <p className="text-accent text-xs tracking-[0.3em] uppercase font-body mb-3">Our Team</p>
            <h2 className="text-2xl md:text-3xl font-serif text-foreground">Department Doctors</h2>
          </div>
        </ScrollAnimationWrapper>
          ...
          <div ref={scrollRef} className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
            {doctors.slice(0, 12).map((doc) => (
              <Link
                key={doc.id}
                to={`/doctors/${doc.id}`}
                className="min-w-[220px] bg-popover border border-border/50 rounded-2xl overflow-hidden hover:border-primary/30 hover:shadow-md transition-all group flex-shrink-0"
              >
                <div className={`${doc.color} h-32 flex items-center justify-center relative`}>
                ...
                <div className="p-4">
                  ...
                  <p className="font-body text-xs text-muted-foreground mt-1">{lang === "ar" ? doc.titleAr : doc.title}</p>
...
const DepartmentDetail = () => {
  ...
  const deptDoctors = allDoctors.filter(
    (doc) =>
      doc.department.toLowerCase().includes(dept.name.toLowerCase().split("&")[0].trim().substring(0, 10)) ||
      dept.name.toLowerCase().includes(doc.department.toLowerCase().split("&")[0].trim().substring(0, 10))
  );
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-5 h-5 text-muted-foreground transition-transform ${
                          expandedSub === sub.slug ? "rotate-180" : ""
                        }`}
                      />
                    </button>
